<template>
  <div class="note-list">
    <div
      class="single-member effect-2"
      v-for="(item, index) in data"
      :key="index"
    >
      <div class="member-image">
        <img :src="item.cover" alt="Cover Image" />
      </div>
...
<script>
import dataFile from '../data/Coursera.json'
...
.effect-2 .member-image {
  border-bottom: 5px solid $courseraColor;
  height: 205px;
  overflow: hidden;
  width: 100%;
  transition: 0.4s;
...
.effect-2 .social-touch {
  background-color: $courseraColor;
  float: left;
  left: 0;
  bottom: 0;
  overflow: hidden;
